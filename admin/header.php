<!DOCTYPE html>
<html lang="en">

<head>
	<meta charset="utf-8">

	<!-- Page resizing -->
	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

	<!-- Page icon -->
	<link rel="icon" href="res/database_logo.png" type="image/png">
	<link rel="shortcut icon" href="res/database_logo.png" type="image/jpg" />

	<!-- Bootstrap core 4.4.1 -->
	<link rel="stylesheet" href="../css/bootstrap.min.css">

	<!-- Custom stylesheet -->
	<link rel="stylesheet" href="../css/style.css">

	<!-- jQuery 3.4.1 -->
	<script src="../js/jquery.min.js"></script>


	<link rel="stylesheet" type="text/css" href="../evo-calendar/css/evo-calendar.css" />
    <link rel="stylesheet" type="text/css" href="../evo-calendar/css/evo-calendar.midnight-blue.css" />
	<!-- Bootstrap bundle js -->
<script src="../js/bootstrap.bundle.min.js"></script>

<!-- jQuery easing 1.4.1 -->
<script src="../js/jquery.easing.min.js"></script>

<!-- Custom JS -->
<script src="../js/customjs.js"></script>
<script src="../evo-calendar/js/evo-calendar.js"></script>

	<title>Welcome to PoS and NER Dataset</title>
</head>
